...
Phase 4 currently has passing local contract/API tests and build checks. On 2026-09-15, Azure revision `virtuapet-staging-api--a9f4854-http` was deployed from the immutable commit image, migrations 001-004 were applied through private-network jobs, and the revision was proven to connect as the least-privilege `virtuapet_app` role. The staging revision receives 100% of API traffic and uses HTTP health, readiness, and startup probes. Prior revisions remain at 0% for rollback. Backup restore, transaction-bound RLS, authenticated end-user workflows, tenant-isolation drills, and the `api.virtuapet.com` DNS/custom-domain binding remain open. See `docs/product/PHASE_4_ACCEPTANCE.md` for the evidence boundary.
