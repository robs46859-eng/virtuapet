...
Cloud review on 2026-09-16 found no completed integration provisioning to accept or repair. Azure still runs `virtuapet-staging-api--a9f4854-http` at 100% traffic from commit `a9f4854`; its environment has no Layer8 integration variables, its secret list contains only the existing database and clinical-twin secrets, and the only recorded migration execution predates migration 005 and applied migrations 001–004. Fresh public probes returned 200 for `/healthz`, 200 for `/readyz`, 401 for an anonymous protected route, and 200 for the Hostinger site. No signing key, identity-link encryption key, tenant map, or dedicated `virtuapet:policy` credential was found or claimed.
...
Next operational sequence: deploy and run migration 005 through migration authority; provision operator-managed signing and encryption keys; create explicit tenant mappings and dedicated per-tenant `virtuapet:policy` keys; configure Redis and both services in isolated staging; run two-tenant allowed/denied/expiry/revocation drills; then promote immutable images only after rollback and observability checks. Keep `LAYER8_POLICY_ENABLED` and `LAYER8_IDENTITY_LINKS_ENABLED` false until those gates pass.
