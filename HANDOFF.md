...
Next operational sequence: restore the existing Layer8 Render API and obtain its operator-controlled configuration path; identify real approved staging organizations on both sides; create a VirtuaPet-owned managed secret store and separate signing/encryption keys; create explicit mappings and dedicated per-tenant `virtuapet:policy` keys; confirm Layer8 Redis readiness; deploy both immutable images to isolated staging revisions; run two-tenant allowed/denied/expiry/revocation drills; then promote only after rollback and observability checks. Keep `LAYER8_POLICY_ENABLED` and `LAYER8_IDENTITY_LINKS_ENABLED` false until those gates pass.
