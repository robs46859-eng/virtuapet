...
Next operational sequence: finish Layer8's Azure-native audit image, worker and `/readyz` acceptance while preserving separate service and database ownership; identify real approved staging organizations on both sides; create separate managed signing/encryption keys; create explicit mappings and dedicated per-tenant `virtuapet:policy` keys; deploy the VirtuaPet immutable image to an isolated revision; run two-tenant allowed/denied/expiry/revocation drills; then promote only after rollback and observability checks. Keep `LAYER8_POLICY_ENABLED` and `LAYER8_IDENTITY_LINKS_ENABLED` false until those gates pass.
