...
const organizationFromRow = (row: Record<string, unknown>): Organization => ({
...
export class PostgresPetRepository implements PetRepository {
  ...
  async findOrganizationById(organizationId: string) { const result=await this.pool.query("SELECT * FROM organizations WHERE organization_id=$1",[organizationId]); return result.rows[0] ? organizationFromRow(result.rows[0]) : undefined; }
